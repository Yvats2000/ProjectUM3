...
const veryHigh = ({cmsData = [], rightNavBar, fundsAmc, totalCounts,  data, filterMaster, pageNo})=>{
    ...
    const metaData = {
        "title" : cmsData.length > 0 && cmsData[0].meta_title && cmsData[0].meta_title != '' ? `${cmsData[0].meta_title}${pageNo != 0 ? ' | Page-'+pageNo : ''}` : `${amcName} Very High MF to Invest in 2023${pageNo != 0 ? ' | Page-'+pageNo : ''}`,
        "description" : cmsData.length > 0 && cmsData[0].meta_description && cmsData[0].meta_description != '' ? `${cmsData[0].meta_description}${pageNo != 0 ? ' | Page-'+pageNo : ''}` : `List of ${amcName} - Very High MF for investment in 2023. Check ${amcName} Very High MF latest NAV, historical returns, performance, ratings by CRISIL, morningstar etcInvesting${pageNo != 0 ? ' | Page-'+pageNo : '.'}`,
        "url" : `${process.env.BASE_URL}/mutual-funds/${fundsAmc}/very-high`
    }
    const FaqArray = [];
    ...
    },
    {
        ...
        "itemListElement": [{
            ...
            "position": 4, 
            "name": "Very High",
            "item": "${process.env.BASE_URL}/mutual-funds/${fundsAmc}/very-high"  
        }]
    }`
    }
    ]
    ...
    const breadCrumbLinks = [
        ...
        {   
            "text": `Very High`,
            "path": `/mutual-funds/${fundsAmc}/very-high`, 
            "class": ""
        }
    ];
    return (
        <>
            ...
            <Category fundsAmc={fundsAmc} titleText={`${amcName} Very High`} risk={'Very High'} totalCounts={totalCounts} tableData={data} cmsData={cmsData} breadCrumbLinks={breadCrumbLinks} filterMaster={filterMaster} />
        </>
    );
}
export async function getServerSideProps(context) {
    const fundsAmc = context.params.amc;
    const cmsData = await getAmcCategory(fundsAmc,'very-high');
    const data = await getAmc(`amc/${fundsAmc}/very-high`, 'table');
