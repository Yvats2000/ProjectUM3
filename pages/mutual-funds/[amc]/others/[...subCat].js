...
const subCategory = ({cmsData = [], rightNavBar, fundsAmc, totalCounts, data,masterDefault, filterMaster,categoryName, pageNo})=>{
    ...
    const metaData = {
        "title" : cmsData.length > 0 && cmsData[0].meta_title && cmsData[0].meta_title != '' ? `${cmsData[0].meta_title}${pageNo != 0 ? ' | Page-'+pageNo : ''}` : `${amcName} Others-${amcCategrory} MF to Invest in 2023${pageNo != 0 ? ' | Page-'+pageNo : ''}`,
        "description" : cmsData.length > 0 && cmsData[0].meta_description && cmsData[0].meta_description != '' ? `${cmsData[0].meta_description}${pageNo != 0 ? ' | Page-'+pageNo : ''}` : `List of ${amcName} - Others-${amcCategrory} MF for Investment in 2023. Check ${amcName}-${amcCategrory} Others MF latest NAV, historical returns, performance, ratings by CRISIL, morningstar etcInvesting${pageNo != 0 ? ' | Page-'+pageNo : '.'}`,
        "url" : `${process.env.BASE_URL}/mutual-funds/${fundsAmc}/others/${categoryName}`
    }
    const FaqArray = [];
    ...
    },
    {
        ...
        "itemListElement": [{
            ...
            "position": 5, 
            "name": "${amcCategrory}",
            "item": "${process.env.BASE_URL}/mutual-funds/${fundsAmc}/others/${categoryName}"  
        }]
    }`
    }
    ]
    ...
    const breadCrumbLinks = [
        ...
        {   
            "text": `${amcCategrory}`,
            "path": `/mutual-funds/${fundsAmc}/others/${categoryName}`, 
            "class": ""
        }
    ];
    return (
        <>
            ...
            <Category  titleText={`${amcName} Others - ${amcCategrory}`} totalCounts={totalCounts} tableData={data} cmsData={cmsData} breadCrumbLinks={breadCrumbLinks} masterDefault={masterDefault} filterMaster={filterMaster} />
        </>
    );
}
export async function getServerSideProps(context) {
    ...
    const cmsData = await getGeneralContent(`/mutual-funds/${fundsAmc}/other/${category}`);
    const data = await getAmc(`amc/${fundsAmc}/other?category=${category}&limit=10&page=${context.query.page || '1'}&sortKey=${context.query.sortKey || 'return'}&sortOrder=${context.query.sortOrder || 'desc'}`, 'table');
    const filterMaster = await getAmc('amc/master');
    let queryMaster = {
        category_code: "3",
        category_type: [],
        amcSlug: [fundsAmc],
        category:[category[0].split('-').join(' ').replace(/(^\w|\s\w)/g, m => m.toUpperCase())+'-Other'],
        rating:  "",
        sipDuration:  5,
        risk: []
    };
