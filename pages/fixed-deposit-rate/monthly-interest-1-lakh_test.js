...
const monthlyInterest1Lakh = ({cmsData, genericUrl, rightNavBar, bankFdData, topBanks}) => {
  const pageName = genericUrl.split('-').join(' ').replace(/(^\w|\s\w)/g, m => m.toUpperCase());
  const metaData = {
    "title" : cmsData && cmsData.length > 0 ? cmsData[0].meta_title : pageName,
    "description" : cmsData && cmsData.length > 0 ? cmsData[0].meta_description : pageName,
...
export async function getServerSideProps(context) {
    ...
    const bankFdData = await getMaster("monthly-interest-1-lakh");
    ...
    return { props: { cmsData : cmsData, topBanks : topBanks, genericUrl : genericUrl.split('fixed-deposit-rate')[1].substring(1), bankFdData : bankFdData || {} } }
}
export default monthlyInterest1Lakh;
