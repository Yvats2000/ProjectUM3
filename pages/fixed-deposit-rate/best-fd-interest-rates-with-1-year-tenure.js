...
const bestFdInterestRatesWith1YearTenure = ({cmsData, genericUrl, rightNavBar, bankFdData, topBanks}) => {
  const pageName = genericUrl.split('-').join(' ').replace(/(^\w|\s\w)/g, m => m.toUpperCase());
  const metaData = {
    "title" : cmsData && cmsData.length > 0 ? cmsData[0].meta_title : 'Best FD Rates for 1 Year - Latest Fixed Deposit Rates 2023',
    "description" : cmsData && cmsData.length > 0 ? cmsData[0].meta_description : 'Compare banks and get highest interest rates on fixed deposits with terms of one year. Check Best FD Rates for 1 Year tenure from Top Banks on Urban Money.',
...
export async function getServerSideProps(context) {
    ...
    return { props: { cmsData : cmsData, topBanks : topBanks, genericUrl : genericUrl.split('fixed-deposit-rate')[1].substring(1), bankFdData : bankFdData.tenures['365'] || [] } }
}
export default bestFdInterestRatesWith1YearTenure;
