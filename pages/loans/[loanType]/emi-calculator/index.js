...
const EMICalculatorType = ({rightNavBar,cmsData, blogsData}) => {
    const router = useRouter();    
    const loanType = router.query.loanType.split('-').join(' ').replace(/(^\w|\s\w)/g, m => m.toUpperCase());
    const calculatorType = 'EMI Calculator';
    const metaData = { 
        ...
        "keywords" : "",
        "url" : `${process.env.BASE_URL}/loans/`+router.query.loanType+ `/`+`emi-calculator`
    }
    const FaqArray = [];
        ...
        },
        {
            __html: `{
              ...
              "itemListElement": [{
                ...
                "position": 3, 
                "name": "${loanType} ${calculatorType}",
                "item": "${process.env.BASE_URL}/loans/${router.query.loanType}/emi-calculator"  
              }]
            }`
        }
      ]
    ...
    return (
        <>
            ...
            <ParentFile blogsData={blogsData} cmsData={cmsData} loanType={router.query.loanType} rightNavBar={rightNavBar} calculatorType={'emi-calculator'} />
        </>
    );
}
export async function getServerSideProps(context) {
  let loanType = context.params.loanType;
  let calculatorType = 'emi-calculator';
...
export default EMICalculatorType;
