...
const PrePaymentCalculatorType = ({rightNavBar,cmsData, blogsData}) => {
    const router = useRouter();    
    const loanType = router.query.loanType.split('-').join(' ').replace(/(^\w|\s\w)/g, m => m.toUpperCase());
    const calculatorType = 'Pre Payment Calculator';
    const metaData = { 
        ...
        "keywords" : "",
        "url" : `${process.env.BASE_URL}/loans/`+router.query.loanType+ `/`+`pre-payment-calculator`
    }
    const FaqArray = [];
        ...
        },
        {
            __html: `{
              ...
              "itemListElement": [{
                ...
                "position": 3, 
                "name": "${loanType} ${calculatorType}",
                "item": "${process.env.BASE_URL}/loans/${router.query.loanType}/pre-payment-calculator"  
              }]
            }`
        }
      ]
    ...
    return (
        <>
            ...
            <ParentFile blogsData={blogsData} cmsData={cmsData} loanType={router.query.loanType} rightNavBar={rightNavBar} calculatorType={'pre-payment-calculator'} />
        </>
    );
}
export async function getServerSideProps(context) {
  let loanType = context.params.loanType;
  let calculatorType = 'pre-payment-calculator';
...
export default PrePaymentCalculatorType;
