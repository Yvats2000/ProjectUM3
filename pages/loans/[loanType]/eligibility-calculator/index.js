...
const EligibilityCalculatorType = ({rightNavBar,cmsData, blogsData}) => {
    const router = useRouter();    
    const loanType = router.query.loanType.split('-').join(' ').replace(/(^\w|\s\w)/g, m => m.toUpperCase());
    const calculatorType = 'Eligibility Calculator';
    const metaData = { 
        ...
        "keywords" : "",
        "url" : `${process.env.BASE_URL}/loans/`+router.query.loanType+ `/`+`eligibility-calculator`
    }
    const FaqArray = [];
        ...
        },
        {
            __html: `{
              ...
              "itemListElement": [{
                ...
                "position": 3, 
                "name": "${loanType} ${calculatorType}",
                "item": "${process.env.BASE_URL}/loans/${router.query.loanType}/eligibility-calculator"  
              }]
            }`
        }
      ]
    ...
    return (
        <>
            ...
            <ParentFile blogsData={blogsData} cmsData={cmsData} loanType={router.query.loanType} rightNavBar={rightNavBar} calculatorType={'eligibility-calculator'} />
        </>
    );
}
export async function getServerSideProps(context) {
  let loanType = context.params.loanType;
  let calculatorType = 'eligibility-calculator';
...
export default EligibilityCalculatorType;
