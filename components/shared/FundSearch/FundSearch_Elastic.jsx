...
import { getAmcApi,getFundsSearch } from '../../../services/mutualFunds';
import Validation from '../../../form/Validation';
import Image from "next/image";
import styles from './FundSearch.module.css';
export const FundSearchElastic = ({onlyAMC = false,innerPage=false}) => {
  ...
  const getBalance = async() => {
    if (search.length > 2) {
      let amcList = await getFundsSearch(`${search}`);
            ...
            <ul>
              {suggestionsList.map((item, index) => (  
                  <li key={index}>
                    ...
                    <figure><img src={process.env.IMAGE_BASEURL + `/amc/icon/${item.amcLogo}`} style={{ width: "21px", height: "21px" }} alt={item.schemeSlug.length && item.amcSlug.length>0?item.scheme_name:item.amc}/></figure>
                    {item.schemeSlug.length && item.amcSlug.length > 0 ?item.scheme_name:item.amc}
                    </div>
