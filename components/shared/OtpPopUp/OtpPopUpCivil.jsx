...
export const OtpPopUpCivil = ({ mobileNo, showError,optHandle, OtpValid,otpmsg,sendOtpAgain, OtpSubmit, isOpen, setPopUpOpen,onlyMobile=false,resendEligible=true }) => {
    ...
    <div className={`${styles.popUpBox} ${isOpen ? styles.active : ''}`}>
      <div className={`${styles.popUpWindow} ${styles.popUpThanks}`}>
        <div className={styles.headingBox}>
          <p className={`${showError?'textef3b51':'text231f20'} font20 mb30 textBlack bottomborderf5a623 ${styles.popupHeading} fontBold lineHeight36`}>Verify your number</p>
          <span className={`${styles.close} cursorPointer`} onClick={(e) => setPopUpOpen(false)}></span>
        </div>
        <div className={styles.thanksMessage}>
          <p className="font18 textBlack fontBold mb15">Enter 6 Digit OTP</p>
          <p className="w100 font14 lineHeight22">{otpmsg}</p>
          ...
          {showError && <p className={`${showError?'textef3b51':'text32356b'} fontBold mb10 font12 mra`}>Please Enter OTP!</p>}
          <div className={styles.powerdByBox}>
            <div>
              {resendEligible&& <p className="mra font12 lineHeight22 textLink cursorPointer" onClick={(e)=>sendOtpAgain(e)}>Send the code again</p>}
              <p className="mra font12 lineHeight22 mb15 textLink cursorPointer" onClick={(e) => setPopUpOpen()}>Change mobile number{!onlyMobile && ''}</p>
            </div>
            <figure><Image width={71} height={44} src="/assets/images/powerdBy.jpg" className="imgResponsive"/></figure>
          </div>
          
          <Button className={`btn btn-primary textCenterSm font14 bnt100 textCenter ${styles.btnAlign}`} onClick={OtpSubmit}>Submit <em className="icon-arrow-right font14"></em></Button>
        </div>
      </div>
    </div>
  );
